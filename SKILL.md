...
生成【本周完成的任务】时，每条任务的第一行必须是五标签定长行。五个字段严格定义如下：

| 字段 | 含义 | 取值规则 | 空值写法 |
|------|------|---------|---------|
| **场景** | 一句话任务场景（做什么、为谁/哪里用） | 中文名词短语，≤ 25 字 | 不允许空 |
| **产出** | 本任务具体可交付的成果 | 逗号或顿号分隔多项 | 不允许空 |
| **迭代** | 有效迭代轮数（非闲聊轮） | 纯数字；估算加 `~` 前缀 | 不允许空 |
| **批量** | 批量处理对象数 | 整数 | `—`（em-dash U+2014） |
| **工作空间** | workspace 名 | 与扫描结果一致 | 不允许空 |

**强制规则**：
- 顺序固定：场景 → 产出 → 迭代 → 批量 → 工作空间
- 分隔符统一用 ` | `（空格 竖线 空格）
- 标签全部加粗 `**字段**`
- 不允许省略任何字段；确实没内容按"空值写法"填

**防刷分硬规则**：
- **迭代**只计文件变更/改稿轮数，不计闲聊轮
- **批量**必须有可数实证（可数到的文件数/记录数），不得估算
- 同一交付物聚合为 1 条，不得拆分（沿用 v2.4.1）
- Skill 首开按交付物 L 分，小版本迭代降档

**深度任务**（迭代 ≥ 20 或 批量 ≥ 100 或 产出 ≥ 3 项）**必须附 3-N 行自由叙述**，保留方法论、技术突破、团队价值等叙事；轻量任务可省略第二行。
...
【本周完成的任务】
（逐条列出；每条任务输出两行——第一行为五标签定长行，第二行为自由叙述）
（格式、字段、few-shot 样例详见下文"任务清单输出格式"小节）
...
#### 任务清单输出格式（v3.3.0 起硬约束）

第一行必须严格遵循以下格式（顺序、分隔符、**粗体标签**都不可变）：

```
**场景** {一句话场景描述} | **产出** {交付物，多个用顿号} | **迭代** {纯数字或 ~数字} | **批量** {数字或 —} | **工作空间** {workspace 名}
```

第二行为自由叙述，长度不限：
- **深度任务**（迭代 ≥ 20 或 批量 ≥ 100 或 产出 ≥ 3 项）**必须写**，3-N 行，保留方法论、技术突破、团队价值等叙事
- 轻量任务可以省略整个第二行

Few-shot 样例（3 条，注意样例 3 只有一行——轻量任务合法省略第二行）：

~~~markdown
1. 三语 AI Practitioner 课程体系建设
   **场景** 三语课程工程化 | **产出** H5P 查看器、中英粤三版 PPTX、180 道题库、覆盖率分析工具 | **迭代** 80 | **批量** 180 | **工作空间** ai-practitioner-course

   本周完成了三语课程工程化的完整闭环：H5P 查看器解决跨浏览器兼容；粤语版加 Jyutping 注音已共享 sinjunli；180 题对标 Prometric 标准，双维度校准（通过率/区分度）；覆盖率工具输出 HTML 报告，jianiili/johnnyxsu 复用中。方法论已可沉淀为独立 Skill 候选。

2. 培训认证工单批量分析
   **场景** 工单根因与分类 | **产出** 11 维度分析报告、Skill v1 | **迭代** 66 | **批量** 600 | **工作空间** tam-toolkit

   10 Agent 并行处理 600 条工单，单次峰值吞吐约 20 条/分钟；分类体系含根因链路（产品/流程/人员）、优先级、解决路径等 11 维；报告已在培训认证中心周会使用。

3. TAPD 八维度评估 Skill 使用
   **场景** PRD 评估 | **产出** 评估报告 | **迭代** 5 | **批量** — | **工作空间** tapd-assessment
~~~
...
- **批量同质工作合并为 1 条**，在五标签里用 `**批量**` 字段体现规模。
  - ✅ 正确：`**场景** 客户工单答复 | **产出** 工单回复、常见问题归纳 | **迭代** 40 | **批量** 600 | **工作空间** tam-toolkit`
  - ❌ 错误：把 600 条工单拆成 600 行
- **同一产出的多语言/多版本/多轮迭代合并为 1 条**，在 `**产出**` 字段里用顿号列出变体。
  - ✅ 正确：`**场景** TCCA 考纲撰写 | **产出** 中文考纲、英文考纲 | **迭代** 12 | **批量** — | **工作空间** training-cert`
  - ❌ 错误：把"中文版""英文版""大纲 review"拆成 3 条
- **同一项目的连续多日推进合并为 1 条**，在自由叙述段里注明跨天。
  - ✅ 正确：`**场景** 客户 POC 部署方案 | **产出** 部署方案文档 | **迭代** 18 | **批量** — | **工作空间** presales`
...
- **v3.3.0** (2026-05-03)：**【新增】任务清单五标签定长行契约**。每条任务第一行严格遵循 `**场景** | **产出** | **迭代** | **批量** | **工作空间**` 定长格式，第二行为不限长自由叙述（深度任务必写）。① Step 5 开头新增「五标签字段契约」小节，明文化五字段取值规则、空值写法、强制顺序；② Step 5 任务清单段替换旧的 `任务描述 · 使用能力 · 所属项目` 格式，给出 3 条 few-shot 样例（含一条轻量任务无第二行合法示例）；③ 新增防刷分硬规则：迭代只计有效改稿轮（非闲聊）、批量必须有可数实证、同交付物聚合不拆分；④ 深度任务阈值：迭代 ≥ 20 或 批量 ≥ 100 或 产出 ≥ 3 项，必须附 3-N 行叙述；⑤ 兼容性：不动 frontmatter、不动 Step 0-4 数据采集、不动 Step 6 邮件发送、沿用 v2.4.1 任务聚合规则；⑥ 成员端：自动拉新版后下次生成即新格式，无需手动配置。动机：配合管理侧月度 B 层复杂度分析自动提取 L1-L5 打分，量化 + 叙事并存。
