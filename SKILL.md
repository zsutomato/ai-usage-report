...
0. **版本闸门（先于一切，重入安全 / re-entrancy safety）**：每一次进入本 skill —— **包括首次触发、被打断后的 `continue`、二次/多次接力续跑、automation 重跑** —— 都必须把 Step 0.3 的 OTA 版本检查当成**第一动作重新执行一遍**，禁止以"我已经过了准备阶段 / 我在流程中段"为由跳过。若本地版本 < 远端版本：**人工模式必须立即停止、输出新版本提醒块、要求用户重新触发**，绝不允许带着旧版 prompt 继续生成或发送。此外，在「生成报告前」与「发送邮件前」两个不可逆节点，必须各做一次「加载版本 vs 本地文件版本」一致性自检（见 Step 0.3.1 与 Step 4.3a）。
0.5. **硬合规发送闸门（hard-compliant gate，fail-closed，通道无关）**：邮件是不可逆动作，其版本一致性校验**不再依赖 agent 自觉执行 `assert`**。发送权限的唯一来源是 Step 4.3a 的 `send_report_gate.sh` 脚本——该脚本**自己**读盘取 `LOCAL_VER`、与 agent 传入的 `LOADED_VER` 比对，**只有相等时才打印一次性 `SEND_OK:` 放行信号**。**禁止 agent 在未拿到 `SEND_OK` 时直接调用任何发送动作**（qq-mail 的 `SendMessage` 或 agent-mail 的 `agently-cli +send` 均禁止）。这样「跳过校验」不再等于「绕过校验放行」，而是「脚本没放行→根本发不出去」：failure mode 从 fail-open 翻转为 fail-closed。脚本 `exit 非 0`（漂移/前置不满足）即视为不可发送，agent 不得自行改用直接发送兜底。
...
5. **发送前必须执行格式校验 gate + 硬合规版本闸门**。subject/body/task format 任一不通过，不得进入邮件发送（两种通道共用同一 gate）；且必须先拿到 Step 4.3a `send_report_gate.sh` 的 `SEND_OK` 放行（见红线 0.5），未放行不得发送。
...
> ⚠️ **重入安全（re-entrancy）——本步骤必须在每个续跑点重新执行**：
> 本 skill 经常被打断后通过 `continue` 续跑，或在多轮接力（改收件人 / 删 CC / 切换发送通道 / 重发等）中被反复进入。Agent **绝不能**因为"上一轮已经做过 Step 0"就跳过本步骤。
> **判定规则**：进入本 skill 时，先记录本次实际加载的 prompt 版本到 `LOADED_VER`（即你当前正在执行的这份 SKILL.md frontmatter 的 `version`）。只要本 turn 涉及生成或发送周报，就必须重新跑一遍下面的 OTA 检查 + Step 0.3.1 版本一致性自检，无论这是第几次接力。
...
##### 0.3.1 版本一致性自检（fail-loud，生成报告前的硬闸门）

OTA 检查之后、进入 Step 1 之前，必须做一次「本次加载的 prompt 版本」与「本地文件当前版本」的一致性断言。这能抓住"运行中 OTA 已把本地刷新、但本 session 仍在用旧版 prompt"的版本漂移（version drift）。

> ⚠️ **`LOADED_VER` 绝对不能在本文件里硬编码、也不能用 `grep` 现读磁盘**。
> 原因：本段代码本身会随 SKILL.md 一起被 OTA 分发。若写成 `LOADED_VER="x.y.z"` 字面量，发新版时这行会停在旧值 → 以后每次正常运行都误报漂移；若用 `grep` 现读磁盘，读到的是「可能已被 OTA 改过的文件」，等于自己跟自己比，永远相等、自检失效。
> **正确做法**：`LOADED_VER` 的唯一可信来源是 **agent 在本次进入 skill 的第一时间、从自己上下文里这份 SKILL.md 的 frontmatter `version:` 字段读到的值**（即"我现在正在执行的这份 prompt 的版本"）。Agent 必须在 Step 0 最开始就把这个值记下来（早于任何 OTA 覆盖动作），然后作为运行时变量传给下面的 bash，**禁止让 bash 自己去文件里取它**。

```bash
# === LOADED_VER 由 agent 运行时注入，不是文件字面量、也不是 grep 磁盘的结果 ===
# agent 必须把「本次进入 skill 时从自身上下文 frontmatter 读到的 version」填到这里，
# 该动作要在 Step 0 起点完成，早于 OTA 可能发生的本地文件覆盖。
LOADED_VER="<agent 在 Step 0 起点从当前 prompt frontmatter 读到的 version；切勿手写死某个具体号>"

# LOCAL_VER：磁盘上 SKILL.md 当前的版本（OTA 可能已在本 turn 把它更新过）—— 这个才用 grep 现读磁盘
LOCAL_VER=$(grep -m1 '^version:' "$HOME/.workbuddy/skills/ai-usage-report/SKILL.md" 2>/dev/null | awk '{print $2}')
[ -z "$LOCAL_VER" ] && LOCAL_VER=$(grep -m1 '^version:' "$HOME/.codebuddy/skills/ai-usage-report/SKILL.md" 2>/dev/null | awk '{print $2}')

if [ "$LOADED_VER" != "$LOCAL_VER" ]; then
  echo "❌ FATAL: 版本漂移 — 本次加载 prompt v$LOADED_VER，但本地文件已是 v$LOCAL_VER。"
  echo "→ 本次仍在用旧版规则，可能产生错误结果（如日期 off-by-one）。已停止生成。"
  echo "→ 请重新执行 /ai-usage-report，以加载 v$LOCAL_VER 后再生成。"
  # 人工模式：停止，不得继续生成或发送。
  # automation 模式：不停止，但必须在报告末尾用上面的 OTA 附注块显式声明本次仍按 v$LOADED_VER 执行。
fi
```

规则：人工模式下自检不通过即停止；automation 模式不停止但必须显式附注。**禁止在自检不通过时静默继续。**

> 实现要点：`LOADED_VER` 是"agent 记忆里的版本"，`LOCAL_VER` 是"磁盘现在的版本"。只有两者来源不同，比对才有意义——前者来自 agent 上下文（不随 OTA 改变），后者来自磁盘（可能被 OTA 改变）。这也是为什么 `LOADED_VER` 必须由 agent 注入，而非任何形式的"读文件"。
...
#### 4.3a 硬合规发送闸门 `send_report_gate.sh`（fused gate，fail-closed，通道无关）

> 设计原则（见红线 0.5）：版本一致性校验从「agent 自觉跑的 `assert`」改为「脚本物理前置于发送动作」。脚本里**版本校验与发送授权是同一进程、不可拆分**——不存在「跳过校验直接发送」的可达路径。跳过脚本 = 拿不到放行 = 发不出去（fail-closed），而不是旧版「跳过 = 静默放行」（fail-open）。本闸门**通道无关**：无论 SEND_CHANNEL 是 qq-mail 还是 agent-mail，Phase 1 前都必须先拿到 `SEND_OK`。
>
> ⚠️ 本版采用 **inline heredoc** 方式：agent 在发送前用下面的内容把 `send_report_gate.sh` 写到临时目录再执行，**不随 SKILL.md 分发额外文件**。校验维度仅做 **version drift**（不含日期/星期校验）。

执行顺序（人工模式，且 Step 4.2 全部前置条件 + 4.3 格式 gate 已满足）：

**① agent 把下面脚本写入 `"$TMPDIR/send_report_gate.sh"` 并以 `LOADED_VER` 作为第 1 参数执行。**`LOADED_VER` 用 Step 0.3.1 中 agent 从自身上下文 frontmatter 读到的同一个运行时值，**作为命令行参数传入，禁止让脚本自己去 grep 取它**。

```bash
#!/usr/bin/env bash
# send_report_gate.sh —— 授权周报发送的唯一闸门（通道无关）。fail-closed：
# 版本漂移或参数缺失一律 exit 非 0，且不打印 SEND_OK，调用方据此判定"不可发送"。
set -euo pipefail

LOADED_VER="${1:-}"                       # 由 agent 从自身上下文注入，必须非空
[ -n "$LOADED_VER" ] || { echo "❌ GATE FAIL: 缺少 LOADED_VER（agent 未注入加载版本）" >&2; exit 2; }

# LOCAL_VER 由脚本自己读盘（OTA 可能已在本 turn 刷新过本地文件）——这是唯一允许的读盘点
LOCAL_VER=$(grep -m1 '^version:' "$HOME/.workbuddy/skills/ai-usage-report/SKILL.md" 2>/dev/null | awk '{print $2}')
[ -z "$LOCAL_VER" ] && LOCAL_VER=$(grep -m1 '^version:' "$HOME/.codebuddy/skills/ai-usage-report/SKILL.md" 2>/dev/null | awk '{print $2}')
[ -n "$LOCAL_VER" ] || { echo "❌ GATE FAIL: 读不到本地 SKILL.md 版本" >&2; exit 3; }

if [ "$LOADED_VER" != "$LOCAL_VER" ]; then
  echo "❌ GATE FAIL: 版本漂移 — 加载 v$LOADED_VER 但本地 v$LOCAL_VER。禁止发送。" >&2
  echo "→ 请重新执行 /ai-usage-report 以加载 v$LOCAL_VER 后再发送。" >&2
  exit 1                                  # fail-closed：不打印 SEND_OK
fi

# 唯一的"放行"出口：版本一致才走到这里
echo "SEND_OK:$LOCAL_VER"
```

**② 解析脚本输出，只认 `SEND_OK`：**
- 脚本 `exit 0` 且 stdout 以 `SEND_OK:` 开头 → 视为放行，继续所选通道（4.4 / 4.5）的 Phase 1。
- 任何其他情况（非 0 退出码 / 无 `SEND_OK` 行 / 脚本执行失败）→ **一律视为不可发送，停止邮件发送**，向用户报告脚本的 stderr 原因。**禁止退回直接 `SendMessage` 或 `agently-cli +send` 兜底。**

> 为什么这是 hard-compliant 而非 soft：旧版 `assert LOADED_VER == LOCAL_VER` 是 agent「自觉执行才有效」的语句，被接力/打断漏掉就 fail-open。现在版本校验在脚本进程内、且是发送授权的前置；agent 唯一能拿到放行信号的方式就是真的跑脚本并拿到 `SEND_OK`。漏跑脚本 → 拿不到 `SEND_OK` → 按 ② 必须停止。
>
> 诚实边界：inline heredoc + 独立发送调用下，脚本本身不能 100% 阻止一个「故意伪造 `SEND_OK` 字符串」的 agent。要做到真正 agent-proof（运行时强制、与 agent 自觉无关），见 4.6 的 `PreToolUse` hook 可选层。

拿到 `SEND_OK:$LOCAL_VER` 后，确认预览必须额外展示：

```text
✅ 版本闸门：send_report_gate.sh 返回 SEND_OK:v{LOCAL_VER}（加载 v{LOADED_VER} 一致，无漂移）
```
...
**前置硬条件（红线 0.5）**：只有 Step 4.3a 的 `send_report_gate.sh` 返回 `SEND_OK:` 后，才允许进入本节 Phase 1。未拿到 `SEND_OK` 一律停止，不得直接调用 `SendMessage`。
...
**前置硬条件（红线 0.5）**：只有 Step 4.3a 的 `send_report_gate.sh` 返回 `SEND_OK:` 后，才允许进入本节 Phase 1。未拿到 `SEND_OK` 一律停止，不得直接调用 `agently-cli +send`。
...
#### 4.6 （可选，最强）PreToolUse hook —— 运行时强制层（通道无关）

若需要**真正 agent-proof**的硬合规（不依赖 agent 是否愿意跑脚本），在 host 侧配置一个针对两个发送工具（qq-mail 的 `SendMessage`、agent-mail 的 `agently-cli +send`）的 `PreToolUse` hook：hook 在发送动作实际执行**之前**由运行时触发，自行比对 `LOADED_VER`/`LOCAL_VER`（或检查 4.3a 脚本产出的一次性 `SEND_OK` 凭据），漂移即 **deny/veto** 该工具调用。这是唯一不经 agent 自觉、由运行时兜底的层。

> 注意：hook 配置 schema 随 WorkBuddy 版本而异，且属 host 级配置（非本 skill 文件内）。本 skill 默认只内置 4.3a 的 fused gate（自包含、无需改 host 配置）；4.6 作为「最大安全」可选项由用户按需开启。配置前请先确认当前 WorkBuddy 的 hook schema。
...
- **v3.5.3** (2026-06-29)：在 v3.5.2 双发送通道之上叠加**重入安全 + 硬合规 fail-closed 发送闸门**（两层守卫，通道无关）。**Layer 1 重入安全（re-entrancy）**：① 新增红线 0——OTA 版本检查升级为「每个续跑点（continue / 多次接力 / 切换通道 / automation 重跑）必须重新执行的第一动作」，禁止以"已过准备阶段"为由跳过；② Step 0.3 顶部加重入安全说明；③ 新增 Step 0.3.1「版本一致性自检」fail-loud 闸门——比对 `LOADED_VER`（本次实际加载的 prompt 版本，由 agent 进入 skill 第一时间从自身上下文 frontmatter 注入，**严禁硬编码、严禁 grep 磁盘**）与磁盘 `LOCAL_VER`（OTA 可能已刷新），抓住"运行中 OTA 已刷新本地、但本 session 仍用旧版 prompt"的版本漂移，杜绝带旧版 bug 逻辑跑完发错报（如日期 off-by-one）。**Layer 2 硬合规发送闸门（hard-compliant，fail-closed）**：① 新增红线 0.5——邮件发送权限唯一来源是 Step 4.3a 的 `send_report_gate.sh`，版本校验与发送授权在同一脚本进程内、不可拆分，**禁止 agent 未拿 `SEND_OK` 时直接发送**（qq-mail 的 `SendMessage` / agent-mail 的 `agently-cli +send` 均禁止）；② 新增 Step 4.3a inline heredoc 脚本（自包含、不随 SKILL.md 分发额外文件），脚本自己读盘取 `LOCAL_VER`、与 agent 传入的 `LOADED_VER` 比对，仅相等时打印一次性 `SEND_OK:` 放行，漂移/缺参一律非 0 退出且不打印 `SEND_OK`；两条通道 Phase 1 前都必须先拿到 `SEND_OK`（failure mode 从 fail-open 翻转为 fail-closed）；③ 校验维度仅保留 version drift（不含日期/星期）；④ 新增 4.6 可选 `PreToolUse` hook 层，作为唯一 agent-proof（运行时强制、不依赖 agent 自觉）的最强兜底，默认不启用。诚实边界：inline heredoc + 独立发送调用下脚本无法 100% 阻止"伪造 `SEND_OK` 字符串"的恶意/失控 agent，真正 agent-proof 须启用 4.6 hook。**除新增两层守卫外，v3.5.2 的双发送通道与所有既有逻辑保持不变。**
